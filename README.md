...
| 内容 | 接口与状态 |
|---|---|
| 主模型 DB1 | 构件、坐标、GUID、属性、装配、组件、螺栓、焊缝、切割/布尔关系；现代构件轴向偏移与深度/平面位置记录，见 [位置证据](docs/PART_POSITION.zh-CN.md) |
...
`tests/corpus.json` 记录 587 个外部文件的固定提交 URL、大小、SHA-256 和 1,172 个用例。第三方模型不随仓库分发。需要 Python 3.11+：
