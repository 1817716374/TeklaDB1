...
`tests/corpus.json` 记录 608 个外部文件的固定来源 URL、大小、SHA-256 和 1,335 个用例。第三方模型不随仓库分发。需要 Python 3.11+：
