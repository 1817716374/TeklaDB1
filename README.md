...
`tests/corpus.json` 记录 587 个外部文件的固定提交 URL、大小、SHA-256 和 1,243 个用例。第三方模型不随仓库分发。需要 Python 3.11+：
